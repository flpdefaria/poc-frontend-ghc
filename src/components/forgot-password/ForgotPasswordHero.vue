...
const steps = [
  { icon: 'pi pi-envelope', text: 'Enter your registered email address' },
  { icon: 'pi pi-inbox', text: 'Check your inbox for the reset link' },
  { icon: 'pi pi-lock', text: 'Set a new password and sign back in' }
]
</script>

<template>
  <section class="flex flex-col gap-6 max-w-[30rem] lg:text-left text-center items-center lg:items-start mx-auto lg:mx-0">
    <Tag value="Password recovery" rounded class="lg:self-start self-center eyebrow" />
    ...
    <p class="text-[1.05rem] leading-relaxed hero-copy">
      No worries — it happens to everyone. Just enter your email and we'll send you a link to reset it right away.
    </p>

    <ul class="flex flex-col gap-3.5 w-full items-start lg:items-start justify-center lg:justify-start">
      <li v-for="step in steps" :key="step.text" class="flex items-center gap-3.5 text-sm feature-text">
